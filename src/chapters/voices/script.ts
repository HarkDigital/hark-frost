...
/*
 * FINGER WRITING — each client's name (and company, smaller) as a monoline
 * "fingertip" stroke that can be written into the fog stroke by stroke.
 *
 * The name is set in Schibsted Grotesk italic on a canvas, thinned to a 1px
 * skeleton (Zhang–Suen), and every skeleton pixel is given a WRITE TIME: its
 * geodesic distance along the stroke from where the finger starts that glyph
 * (Dijkstra over the skeleton), glyphs left to right with a short pen lift in
 * between. A nearest-skeleton distance transform then spreads (distance,
 * write time) to every pixel. The fog shader clears a pixel when
...
/** wait (briefly) for the italic display face; the skeleton is taken from it */
export async function scriptFontsReady(): Promise<boolean> {
  if (!document.fonts?.load) return false
  try {
    await Promise.race([
      Promise.all([
        document.fonts.load(`italic ${NAME_WEIGHT} 100px ${FAMILY}`, 'Aa'),
        document.fonts.load(`${CO_WEIGHT} 30px ${FAMILY}`, 'AB'),
      ]),
      new Promise((_, reject) => setTimeout(() => reject(new Error('font timeout')), 2500)),
    ])
    return true
  } catch {
    return false
...
function fitNameSize(g: CanvasRenderingContext2D) {
  ...
  g.font = `italic ${NAME_WEIGHT} 100px ${FAMILY}`
  let widest = 1
  for (const t of TESTIMONIALS) widest = Math.max(widest, g.measureText(t.name).width)
  nameSize = Math.min(116, ((MW * 0.9) / widest) * 100)
  return nameSize
}
...
function drawName(g: CanvasRenderingContext2D, text: string, size: number) {
  ...
  g.textAlign = 'center'
  g.font = `italic ${NAME_WEIGHT} ${size}px ${FAMILY}`
...
function binarize(g: CanvasRenderingContext2D): Uint8Array {
...
function order(skel: Uint8Array, pixels: number[], w: number, penLift: number): Stroke {
  ...
  const minX = (c: number[]) => {
    let m = Infinity
    for (const i of c) m = Math.min(m, i % w)
    return m
  }
  comps.sort((a, b) => minX(a) - minX(b))
 ...
 * Restricted to the skeleton's bounding box + margin.
 */
function nearest(stroke: Stroke, w: number, h: number, margin: number) {
...
/** the finger-writing mask for testimonial i (built once, ~20–40 ms) */
export function nameMask(i: number): NameMask {
  ...
  const nn = nearest(name, MW, MH, Math.ceil(rName * 3) + 2)

  drawCompany(g, t.company)
  const coImg = binarize(g)
  const coPx = thin(coImg, MW)
  const co = order(coImg, coPx, MW, rCo * 4)
  const cn = nearest(co, MW, MH, Math.ceil(rCo * 3) + 2)
  ...
  const fill = (res: ReturnType<typeof nearest>, st: Stroke, r: number, ch: number) => {
  ...
  const picks = name.comps.length > 3 ? [Math.floor(name.comps.length * (0.2 + rand() * 0.14)), Math.floor(name.comps.length * (0.64 + rand() * 0.16))] : [0]
  for (const ci of picks) {
    const comp = name.comps[ci]
    if (!comp) continue
    let low = comp[0]
    for (const p of comp) if (((p / MW) | 0) > ((low / MW) | 0)) low = p
    const x = low % MW
    const y = (low / MW) | 0
    // only from the baseline (not from a raised stroke end or a descender)
    if (y < NAME_BASE - size * 0.12 || y > NAME_BASE + size * 0.06) continue
    const y0 = y + rName * 0.6
  ...
  const mask: NameMask = { tex, capName: rName / name.total, capCo: rCo / co.total, radius: rName, drips }
  cache.set(i, mask)
  return mask
}

/** forget cached masks (fonts changed after a fallback build) */
export function resetMasks() {
  for (const m of cache.values()) m.tex.dispose()
  cache.clear()
  nameSize = 0
}
